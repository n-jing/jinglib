...
{
  AABBTree<3> aabb_tree(path);
  size_t num_tri = aabb_tree.get_face_num();
  for (size_t i = 0; i < num_tri; ++i)
  {
    const Matrix<T, Dynamic, Dynamic> tri = aabb_tree.get_face(i);
    vector<Eigen::Matrix<T, Dynamic, Dynamic>> neigh_tri = get_intersect_face(aabb_tree, i);
    for (const auto &itr : neigh_tri)
    {
      ...
      if (is_intersect)
      {
        ...
        write_to_vtk({tri, itr}, "self_intersect_tri.vtk");
        return true;
      } 
    }
...
{
  bool is_a_degenerate = is_triangle_degenerate<Derived>(tri_a);
  bool is_b_degenerate = is_triangle_degenerate<Derived>(tri_b);
  if (is_a_degenerate || is_b_degenerate)
    return true;

  array<int, 3> position_a2b, position_b2a;
  ...
  int is_common = is_common_condition<Derived>(tri_a, tri_b, position_a2b, position_b2a);
  if (is_common == 0)
    return false;
  else if (is_common == 1)
    return true;

  bool is_it = is_triangle_triangle_self_intersect<Derived>(tri_a, tri_b, position_a2b, position_b2a);
  return is_it;
}
...
bool is_triangle_triangle_self_intersect(const Eigen::MatrixBase<Derived> &tri_a, const Eigen::MatrixBase<Derived> &tri_b, const std::array<int, 3> &position_a2b, const std::array<int, 3> &position_b2a)
{
  ...
  {
    bool is_a2b_intersect = is_triangle_to_triangle_intersect<Derived>(tri_a, tri_b);
    bool is_b2a_intersect = is_triangle_to_triangle_intersect<Derived>(tri_b, tri_a);
...
bool is_vert_triangle_intersect(const Eigen::MatrixBase<DerivedV> &v, const Eigen::MatrixBase<DerivedV> &p, const Eigen::MatrixBase<DerivedE> &tri, bool coplanar_v_triangle)
{
  assert(p != v);
...
{
  using T = typename DerivedE::Scalar;
  for (size_t i = 0; i < 3; ++i)
  {
    Matrix<T, 3, 2> et;
    et.col(0) = tri.col(i);
