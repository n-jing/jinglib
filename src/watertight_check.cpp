...
bool is_mesh_watertight(const char *const path)
{
  ...
  function<size_t(const Vector3d &)> HashFunc3 =
    bind(HashFunc<Vector3d, 3>, placeholders::_1);
  function<bool(const Vector3d &, const Vector3d &)> EqualKey3 =
    bind(EqualKey<Vector3d, 3>, placeholders::_1, placeholders::_2);
  ...
  function<size_t(const Vector2st &)> HashFunc2 =
    bind(HashFunc< Vector2st, 2>, placeholders::_1);
  function<bool(const Vector2st &, const Vector2st &)> EqualKey2 =
    bind(EqualKey<Vector2st, 2>, placeholders::_1, placeholders::_2);
  ...
  for (const auto &tri : table_tri)
  {
    for (size_t i = 0; i < 3; ++i)
    {
      ...
      if (map_edge.count(e))
      {
        cerr << "not watertight mesh, multi edge find" << endl;
        Vector3st tri_v_id = map_edge.at(e);
        Matrix3d tri_v;
        Matrix3d tri_cv;
        for (size_t i = 0; i < 3; ++i)
        {
          tri_v.col(i) = table_vert.at(tri[i]);
          tri_cv.col(i) = table_vert.at(tri_v_id[i]);
        }
        // write_to_vtk(vector<MatrixXd>{tri_v, tri_cv},
        //              "not_watertight_tri_multi_edge.vtk");
        return false;
      }
  ...
  for (const auto &tri : table_tri)
  {
    for (size_t i = 0; i < 3; ++i)
    {
      ...
      if (!map_edge.count(e))
      {
        cerr << "not watertight mesh, no opposite edge" << endl;
        Matrix3d tri_v;
        for (size_t i = 0; i < 3; ++i)
          tri_v.col(i) = table_vert.at(tri[i]);
        // write_to_vtk(vector<MatrixXd>(1, tri_v), "not_watertight_tri_non_opposite.vtk");
        return false;
      }
    }
  }
...
bool is_normal_outside(const char *const path)
{
  ...
  const size_t face_num = he_mesh.get_face_num();
  double volume = 0;
  for (size_t i = 0; i < face_num; ++i)
  {
    const Matrix3d &tri = he_mesh.get_tri(i);
    Vector3d ab = tri.col(1) - tri.col(0);
    Vector3d ac = tri.col(2) - tri.col(0);
    Vector3d d = tri.col(0);
    volume += ab.cross(ac).dot(d);
  }
...
int get_triangle_normal_axis_direction(const Eigen::Matrix3d &face, size_t axis)
{
  ...
  double dir =
    orient2d(&project_tri[0][0], &project_tri[1][0], &project_tri[2][0]);
